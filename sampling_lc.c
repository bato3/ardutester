...
// returns measured period, with 6 bits of fraction, or 0 if no resonance found
{
    ...
    for (i = 0; i < 255 - dist; i++)
    {
        ...
        if (delta > 0 && prevdelta <= 0 && i > 2 * dist - 1)
        {
            ...
            if (!sawzero)
            { // stop if the signal has not been zero since previous peak
                if (maxpk > 2)
                    break;
                else
                    goto skip; // if we're still in preliminary search mode: just skip this peak hoping we'll find a real one later
            }
            sawzero = 0;
            prevpeak = a + b;
    ...
    if (ipk < 2)
        return 0; // only one or no peak found
    ipk--;
    unsigned int per = (prevpeakx - firstpeakx + (ipk >> 1)) / ipk; // average period; the (ipk>>1) provides for rounding
    if (firstzero > (per >> 6))
        return 0; // sanity check: first zero should be within first period
    return per;
} /* end of peaksearch */
...
//###################################################################################################
    ...
    // check how long until signal reaches 0: that gives us a first guess of 1/4 of the resonance period (because we apply an impulse, so we start at the maximum of the sinewave)
    for (dist0 = 1; dist0 < 250; dist0++)
        if (uu[dist0] == 0)
            break;
    if (dist0 == 250)
        return; // no periodicity seen, so no valid results
#if (DEB_SAM == 5)
    byte dist00;
    dist00 = dist0;
#endif
...
#define samplingADC_direct (1 << smplADC_direct)
    ...
    if ((par >> smplADC_span) < 4)
    {
        ...
        i = 20;
        while (i--) // equivalent to for (i=0;i<20;i++) but saves 48 bytes of flash????
        {
            if (uu[i] >= uu0[i])
                uu[i] -= uu0[i];
            else
...
void sampling_lc_calibrate()
{
    ...
    do
    {
        ...
        i++;
        if ((lc_cpartmp < 9500) || (lc_cpartmp > 33000))
            i = 0; // unstable or not connected
        if (i > 4)
        { // Cx measurement was stable
