...
uint8_t MeasureInductance(uint32_t *Time, uint8_t Mode)
{
    ...
    {
        Test = (CPU_FREQ / 1000000); /* cycles per �s */

        /* change probes: Gnd -- Rl -- probe-2 / probe-1 -- Vcc */
        ADC_PORT = Probes.ADC_1; /* pull up probe-1 directly */

        /*
         *  delay timer by about 3-4�s to skip capacitive effects of large inductors
    ...
    /* convert counter (MCU cycles) to time (in �s) */
    if (Counter > 0)
    {
        Counter += (CPU_FREQ / 2000000); /* add half of cycles for rounding */
        Counter /= (CPU_FREQ / 1000000); /* divide by frequeny and scale to �s */
    }
...
uint8_t MeasureInductor(Resistor_Type *Resistor)
{
    ...
    else if (Test == 3) /* valid time */
    {
        ...
        if (Time1 > Time2)
            Time1 = Time2; /* lower value wins */
    }

    if (Test != 3)
        Test = 0; /* measurements failed */
    ...
    if (Test == 3)
    {
        ...
        Scale = -6;      /* �H by default */
        Value = Time1;   /* t_stop */
        Value *= Factor; /* * factor (�s * 10^-3) */
