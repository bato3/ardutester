...
#define CNT_ZERO_720 7
            ...
            if (cap.cval_uncorrected.dw > cnt_diff)
                cap.cval_uncorrected.dw -= cnt_diff;
            else
...
/* ********************************************************* */
        ...
        if (inductance[2] < resis[found].lx)
            resis[found].lx = inductance[2]; // use the other polarity
