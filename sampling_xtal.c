...
newff:;
    ...
    do
    {
        ...
        u = imax2 >> 8;
        if (ff == 1)
            break; // one hopes we'll not get such a low ff; this happens with crystals very near half the clockfrequency; for those, we can't properly resolve the aliasing
    ...
    if (xtal_cancel)
        return xtal_cancel;
    if (u & 0x80)
        imax2 = -imax2; // make it negative if that's what we expect based on the coarse measurement
#if ((DEB_UART & 0x20) != 0)
    myuart_putc('o');
    myuart_putc(' ');
    uart_int(dmax);
    ...
    if (isXtal)
    {
        ...
        uint8_t sse = ssd / 2;
        if (sse < 5)
            sse = 5; // ssd<5 is not allowed because the time it takes to send the pulses is included in the ssd time
        while (1)
        {
            for (i = ncumul; i != 0; i--)
            {
        ...
        ph1h = 0x00; // for ceramic resonators; tells the below code not to do alias-correction, i.e., not to subtract the measured freq from 16. This is of course a guess, in case of cer.res. we have no way reliably resolve the aliasing.
    ...
    f = imax2;
    while (f + 32768 < (uint32_t)imax1 * ff)
        f += 65536ul; // older version, takes more flash
