...
unsigned long Get_hFE_C(uint8_t Type)
{
    ...
    if (U_R_b < 10) /* I_b < 14�A -> Darlington */
    {
        ...
        if (U_R_b < 1)
            U_R_b = 1;            /* prevent division by zero */
...
void CheckDiode(void)
{
    ...
    if (U2_Rh <= 10)
        return; /* small resistor or very large cap */
