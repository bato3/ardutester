...
large_cap:
    ...
    if (Flag == 3)
    {
        ...
        Value *= 100;
        if (Mode & FLAG_10MS)
            Value /= 109; /* -9% for large cap */
        else
            Value /= 104; /* -4% for mid cap */
...
uint8_t SmallCap(Capacitor_Type *Cap)
{
    ...
    if (Flag == 3)
    {
        ...
        if (Raw > 2)
            Raw -= 2; /* subtract processing time overhead */
...
void MeasureCap(uint8_t Probe1, uint8_t Probe2, uint8_t ID)
{
    ...
    if (Check.Found == COMP_ERROR)
        return; /* skip check on any error */
