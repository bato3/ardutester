...
Large caps:
- R = 680 + 22 (22 is the internal resistance of the �C for pull-up)
- U_in = 5V
...
large_cap:
  ...
  if (Flag == 3)
  {
    ...
    Cap->Raw = Raw;
    Cap->Value = Value;       /* max. 4.3*10^6nF or 100*10^3�F */
  }
...
uint8_t SmallCap(Capacitor_Type *Cap)
{
   ...
   *  use the �Cs internal 16-bit counter and analog comparator. The counter
  ...
  if (Flag == 3)
  {
     ...
     *  internal bandgap reference if C is 100nF - 20�F.
        ...
        ((Scale == -9) && (Value <= 20000)))
    {
       ...
       *  the voltages of the cap and the bandgap reference. Since the �C
       *  has an internal voltage drop for the bandgap reference the
       *  �C used actually U_bandgap - U_offset. We get that offset by
...
//}
  ...
  if (DiodesFound == 0)
  {
    ...
    if (CompFound == COMP_RESISTOR)
    {
      /* report capacitor for large C (> 4.3�F) */
...
void CheckDiode(void)
{
   ...
   *  - Take care about the internal voltage drop of the �C at the cathode
   *    for high currents.
   ...
   *    use for measuring Vf. The current for Rh is about 10.6�A.
   */
...
void CheckResistor(void)
{
  ...
  U_Ri_L = ReadU_5ms(Probe2_Pin);  /* get voltage at internal R of �C */
  ...
  if (U_Rh_L <= 20)
  {
    ...
    U_Ri_H = ReadU_5ms(Probe1_Pin);     /* get voltage at internal R of �C */
...
unsigned long Get_hfe_c(uint8_t Type)
{
  ...
  unsigned int      Ri;            /* internal resistance of �C */
  ...
  if (U_R_b < 10)             /* I_b < 14�A = Darlington */
  {
