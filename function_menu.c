...
#endif
    {
...
#endif
        {
            ...
            if (func_number == MODE_TRANS)
                break; // return to TransistorTester
...
#define KEYPRESS_LENGTH_10ms 20 /* change frequency only with >200ms key press */
        ...
        if (old_freq != freq_nr)
        {
            ...
            if (freq_nr > MAX_FREQ_NR)
                freq_nr -= (MAX_FREQ_NR + 1);
        ...
        if (key_pressed > KEYPRESS_LENGTH_10ms)
            freq_nr++; // longer key press select next frequency
        if (key_pressed >= 80)
            break;        // more than 0.8 seconds
    }                     /* end for times */
...
{
    if (freq_num > MAX_FREQ_NR)
        freq_num -= (MAX_FREQ_NR + 1);
    if (freq_num == 19)
    {
        // 2 MHz
#undef F_TIM1
