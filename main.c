...
void DisplayValue(unsigned long Value, int8_t Exponent, unsigned char Unit)
{
    ...
    if (Exponent >= -12) /* prevent index underflow */
    {
        ...
        if (Index <= 6)
            Prefix = pgm_read_byte(&Prefix_table[Index]);
    }
    ...
    if (Exponent <= 0) /* we have to prepend "0." */
    {
        ...
        lcd_data('.');
        if (Exponent < 0)
            lcd_data('0'); /* extra 0 for factor 100 */
    }

    if (Offset == 0)
        Exponent = -1; /* disable dot if not needed */
...
uint8_t TestKey(unsigned int Timeout, uint8_t Mode)
{
    ...
    if (Timeout == 0)
        Timeout++; /* prevent negative overrun */
    ...
    while (Mode > 0)
    {
        ...
        if (Mode & 0b10000000) /* timeout enabled */
        {
            ...
            if (Timeout == 0)
                Mode = 0; /* reset all flags on timeout */
        }
    }
...
void Selftest(void)
{
    ...
    while (Test <= 9)
    {
        ...
        while (Counter <= 5)
        {
            ...
            case 7:                        /* internal resistance of �C in pull-down mode */
            ...
            case 8:                         /* internal resistance of �C in pull-up mode */
            ...
            if (Counter > 99)
                TempWord = 0;                 /* disable timeout if skipping test */
...
/* kyrillish LCD character set lacks omega and � */
    ...
    Config.RiH = R_MCU_HIGH;                                  /* Ri of �C in high mode */
    Config.RiL = R_MCU_LOW;                                   /* Ri of �C in low mode */
...
start:
    ...
    if (TempByte1 == 1)
        goto start; /* short key press -> next round */
    else if (TempByte1 == 2)
        goto power_off; /* long key press -> power off */
