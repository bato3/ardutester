...
void ShortCircuit(uint8_t Mode)
{
    ...
    {
        if (Test != 0)
            String = (unsigned char *)Remove_str; /* some shorted */
    }
    else /* create short */
    {
        if (Test != 3)
            String = (unsigned char *)Create_str; /* some unshorted */
    }
    ...
    while (Run == 1)
    {
        ...
        {
            if (Test == 0)
                Run = 0; /* end loop if all removed */
        }
        else /* create short */
        {
            if (Test == 3)
                Run = 0; /* end loop if all shorted */
        }
...
uint8_t TestKey(uint16_t Timeout, uint8_t Mode)
{
    ...
    while (Run)
    {
        ...
        {
            if (Timeout > 5)
                Timeout -= 5; /* decrease timeout by 5ms */
            else
                Run = 0; /* end loop on timeout */
        }
        ...
        if (!(CONTROL_PIN & (1 << TEST_BUTTON))) /* if key is pressed */
        {
            ...
            while (Run) /* detect how long key is pressed */
            {
                if (!(CONTROL_PIN & (1 << TEST_BUTTON))) /* key still pressed */
                {
                    ...
                    if (Counter > 26)
                        Run = 0; /* end loop if 300ms are reached */
                    else
                        MilliSleep(10); /* otherweise wait 10ms */
                }
                else /* key released */
                {
                    Run = 0; /* end loop */
