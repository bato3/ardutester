...
#define StartADCwait()                                                                            \
    ADCSRA = (1 << ADSC) | (1 << ADEN) | (1 << ADIF) | AUTO_CLOCK_DIV; /* enable ADC and start */ \
...
void GetESR()
{
    ...
    for (ii = 0; ii < MAX_CNT; ii++)
    {
        ...
        while (1)
        {
            ...
            if (adcv[2] > 2)
                break; // at least more than two digits required
            wdt_reset();
        }                      // end while (1)
